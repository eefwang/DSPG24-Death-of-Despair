...
# Convert the State variable to uppercase in the year.state.drugs dataset
year.state.drugs <- year.state.drugs %>%
  mutate(State = toupper(State))

# Assuming 'year.state.drugs' is already loaded in your environment
year.state.drugs_1822 <- year.state.drugs %>%
  group_by(State) %>%
  summarise(
    Total_Deaths = sum(Deaths),
    Total_Population = sum(Population),
    drugs_death_rate = (Total_Deaths / Total_Population) * 100000,  # Drugs Death rate per 100,000 population
    .groups = "drop"  # Optional: drop the grouping once summarisation is done
  ) %>%
  na.omit()

print("Summarized data:")
print(str(year.state.drugs_1822))  # Print structure of the summarized data

# Merge the summarized data with the spatial data
states_merged <- merge(states, year.state.drugs_1822, by = "State")
print("Merged data:")
...
pal <- colorBin(
  palette = "viridis",
  domain = states_merged$drugs_death_rate,
  bins = mybins
)
...
map <- leaflet(states_merged) %>%
  addTiles() %>%
  addPolygons(
    fillColor = ~pal(drugs_death_rate),
    fillOpacity = 0.7,
    color = "white",
    weight = 1,
    popup = ~paste("State:", State, "<br>Death rate:", drugs_death_rate)
  ) %>%
  addLegend(
    pal = pal,
    values = ~drugs_death_rate,
    opacity = 0.7,
    title = "U.S. Average Drug Death Rates per 100,000 people (2018-2022)<br><span style='font-size:10px; color: black;'>Data obtained from the National Center for Health Statistics (CDC)</span>",
    position = "bottomleft"
  )
